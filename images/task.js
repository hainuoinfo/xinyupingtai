...
function taskin(mid, isDisWW, btn) {
	...
	if ($("#divIDList input").length == 0) {
		artDialog({
			content: "您还未绑定您的淘宝买号，或您绑定的淘宝买号未启用，请点击此处绑定您的淘宝买号。",
			id: "alarm",
			fixed: true,
			yesText: "立即去绑定",
			lock: true
...
function SelectBuyerAccountNew(mid, btn, $accountList, IsReal) {
	...
	if (Reallen == AllReallen && IsReal == 1) {
		...
		artDialog({
			content: "温馨提示：您所有的买号信誉超过2000或买号已经挂起，无法接手此任务，请绑定新买号！",
			id: "al1",
			fixed: true,
			yesText: "关闭",
			lock: true
		...
		if (isFlowVip) {
			$.post("/ajax/Buyantidownright.php", {
				"taskid": mid
			}, function(result) {
		...
		if (!aId) {
			artDialog({
				...
				if (aId == undefined) {
					artDialog({
						content: "花兔兔温馨提示：请选择好买号再提交!<br/>如果没有买号，请到【会员中心-绑定买号】里面先绑定买号再接任务",
						id: "alarm",
						fixed: true,
						yesText: "立即去绑定",
						noText: "重选买号",
				...
				if ($(btn).attr("id") == "btnAutoAM") $("#reAutoAM").hide();
				CancelAcceptMission(mid);
			});
		} else {
			$(btn).attr("disabled", "disabled");
...
function CancelAcceptMission(mid) {
	var tipsDialog = artDialog({
		content: "正在取消接手任务，请稍候...",
		id: "tips",
		lock: true
	});
	DisabledClose();
	$.post("/ajax/MyTaskOut.php", {
		"taskid": mid
	}, function(result) {
		tipsDialog.close();
		$("#taskList").show();
...
function AutoAMResult(result) {
	if (result.state == 1) {
		artDialog({
			content: "服务器拒绝获取数据。原因：您的登录已失效，或者您已在其他地方登录",
			id: "alarm"
		}, function() {});
		StopAutoAM();
	} else if (result.state == 2) {
		artDialog({
			content: "服务器拒绝获取数据。原因：自动接任务功能每天只能免费接到3个任务，您今日的配额已经接满，请明天再使用。",
			id: "alarm"
		}, function() {});
		StopAutoAM();
	} else {
		if ($("#divIDList input").length == 0) {
			...
			if (result.itemList.length < 10) waitMList = result.itemList;
			else {
...
function RanArr(oArr) {
	if (oArr.length == 1) return oArr[0];
	else return oArr[Math.round(Math.random() * 10000) % oArr.length];
}
