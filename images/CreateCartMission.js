$(document).ready(function(){
    ...
    if(curG<2){
        artDialog({content:"对不起！您的兔粮不够了，发布购物车任务最少需要2个兔粮，请充值购买兔粮或者接任务免费获得兔粮。",id:"alarm",fixed:true,lock:true,yesText:"立即购买兔粮",noText:"马上去接任务"},
...
function addconfirm(){
	...
	if(price_count<1 || point_count<1){
		artDialog({content:"对不起！发布失败，商品价格或者消耗兔粮不得低于1",id:"alarm",fixed:true,lock:true,yesText:"返回修改"},function(){ToMao();});
		return false;
		}
	if(curG<point_count){
