                    ...
                    if ($avatar){
                     ...
                     if ($datas['price'] > 0){
                        ...
                        if ($price <= 40) $point = 1;
                         ...
                         if ($datas['isFMaxBTSCount']) $point += 0.5;
                        ...
                        if (($datas['price'] + ($datas['isExpress']?3:0)) * $count <= $member['attach']['money']){
                             ...
                             return "<script>alert('兔粮不足，发布失败！');window.location.href='/member/BuyPoint/';</script>";
                             }
	     ...
	     if ($task['status'] == 1){
	        ...
	        if (db :: del_id('task', $id)){
	            ...
	            member_base :: addFabudian($uid, $task['point'], $task['type'], '取消淘宝区任务');
	             member_base :: addFabudian($uid, - cfg :: getMoney('sys', 'point_task_del'), $task['type'], '取消淘宝区任务扣除0.2个兔粮');
		     ...
		     if ($task['status'] == 1){
		         ...
		         //if ($sys_debug){//全局启用严格模式
		            ...
		            if ($task['isLimit']){
		             ...
		             if ($task['isScore']){
		                 // 限制接手人积分
		                if ($memberFields['credits'] < $task['scoreLvl'])
		                     return '对不起，您的积分不足以接该任务';
		                 }
		             if ($task['isCredit']){
		                 // 限制接手人信用值
