<?php $_tplModify=filemtime('D:\damaihu\xinyupingtai7\cache\default\founder\i_club.php');if(filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\h.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\header.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\f.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\footer.htm')>$_tplModify){include(template::load_base('D:\damaihu\xinyupingtai7\templates\default\founder\i_club.htm','D:\damaihu\xinyupingtai7\cache\default\founder\i_club.php',true));exit;}?><?php !defined("IN_JB")&&exit("error");$__tplUrl = '/templates/default/founder/';echo '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
';if($method=='list'){echo '
	...
	<table class="tb tb2 fixpadding"><tr class="header"><th>删？</th><th>门派名称</th><th>图标</th><th>简介</th><th>创建时间</th><th>状态</th><th></th></tr>';if($list){foreach($list as $k=>$v){echo '<tr class="hover"><td><input type="checkbox" name="del[]" value="';echo $v['id'];echo '" class="checkbox" /></td><td>';echo $v['name'];echo '</td><td><img src="';echo $weburl2;echo 'img/club/';echo $v['ico'];echo '" /></td><td>';echo $v['remark'];echo '</td><td>';echo date('Y-m-d H:i:s',$v['timestamp']);echo '</td><td>';if($v['status']){echo '正常';}else{echo '待审核';}echo '</td><td>';if(!$v['status']){echo '<a href="';echo $base_url;echo '&method=';echo $method;echo '&verify=';echo $v['id'];echo '" onclick="return confirm(\'确定吗？\')">通过审核</a>';}echo '</td></tr>';}}echo '<tr><td colspan="7"><input type="checkbox" id="checkDel" onclick="check_all(this,\'del[]\')" class="checkbox" /><label for="checkDel">全选</label></td></tr><tr><td colspan="7">';echo $multipage;echo '</td></tr><tr><td colspan="7"><input type="submit" value="提交" class="btn" /></td></tr></table></form>';echo '
';}echo '
';echo '</div>
';echo '</body>
</html>';?>