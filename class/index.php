...
if ($memberLogined) {
	...
	if ($cookie['checkPwd2'])$member['checkPwd2'] = true;
	else $member['checkPwd2'] = false;
	$memberGroup = $userGroups2[$member['groupid']];
	if ($memberGroup['key'] == 'admin') $isAdmin = true;
	$memberLevel = member_credit::getLevelAll($memberFields['credits']);
	if ($memberFields['vip']) $isVip   = true;
	if ($memberFields['vip2']) $isVip2 = true;
	if ($memberFields['flowVip']) $isFlowVip = true;
	if ($memberFields['isEnsure']) $isEnsure = true;
/*
积分对应的同时接手 发布最大数量
0-100 10 10 
101-1000 20 20 
