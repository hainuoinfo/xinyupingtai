<?php $_tplModify=filemtime('D:\damaihu\xinyupingtai7\cache\default\founder\plugins_manage.php');if(filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\h.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\header.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\f.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\footer.htm')>$_tplModify){include(template::load_base('D:\damaihu\xinyupingtai7\templates\default\founder\plugins_manage.htm','D:\damaihu\xinyupingtai7\cache\default\founder\plugins_manage.php',true));exit;}?><?php !defined("IN_JB")&&exit("error");$__tplUrl = '/templates/default/founder/';echo '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
';if($method=='all'||$method=='installed'||$method=='uninstalled'){echo '
';echo '<form method="post" enctype="application/x-www-form-urlencoded" onsubmit="return confirm(\'确定提交吗？\')">';echo $sys_hash_code;echo '
	<table class="tb tb2 fixpadding"><tr class="header"><th>删？</th><th>插件名称</th><th>插件简介</th><th>插件作者</th><th>技术支持</th><th>插件类型</th><th>版本号</th><th></th></tr>';if($list){foreach($list as $k=>$v){echo '<tr class="hover"><td><input type="checkbox" name="del[]" value="';echo $v['folder'];echo '" class="checkbox" /></td><td>';echo $v['title'];echo '</td><td>';echo $v['description'];echo '</td><td>';echo $v['author'];echo '</td><td>';echo $v['support'];echo '</td><td>';if($v['type']=='header'){echo '程序开始运行';}elseif($v['type']=='footer'){echo '程序结束运行';}elseif($v['type']=='marker'){echo '模板调用';}else{echo '未知类型';}echo '</td><td>';echo $v['version'];echo '</td><td>';if($v['installed']){echo '';if($v['suspend']){echo '[<a href="';echo $base_url;echo '&resume=';echo urlencode($v['folder']);echo '" onclick="return confirm(\'您确定要恢复吗？\')">恢复</a>]';}else{echo '[<a href="';echo $base_url;echo '&suspend=';echo urlencode($v['folder']);echo '" onclick="return confirm(\'您确定要暂停吗？\')">暂停</a>]';}echo '[<a href="';echo $base_url;echo '&uninstall=';echo urlencode($v['folder']);echo '" onclick="return confirm(\'您确定要卸载吗？\')">卸载</a>]';}else{echo '[<a href="';echo $base_url;echo '&install=';echo urlencode($v['folder']);echo '" onclick="return confirm(\'您确定要安装吗？\')">安装</a>]';}echo '</td></tr>';}}echo '<tr><td colspan="8"><input type="checkbox" id="checkDel" onclick="check_all(this,\'del[]\')" class="checkbox" /><label for="checkDel">全选</label></td></tr><tr><td colspan="8">';echo $multipage;echo '</td></tr><tr><td colspan="8"><input type="submit" value="提交" class="btn" /></td></tr></table></form>';echo '
';}echo '
';echo '</div>
';echo '</body>
</html>';?>