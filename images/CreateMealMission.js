$(document).ready(function() {
	if (curG < 1) {
		...
		$("#txtPrice").blur(function() {
			...
			if (curGoodsPrice != "") {
				if (curGoodsPrice <= 0) artDialog({
					content: "对不起！输入的商品总价格式错误，请重新输入。",
					id: "alarm"
				}, function() {}, null, function() {
					Init();
					ToMao();
				});
				else ValidPrice(parseFloat(curGoodsPrice));
			} else {
				Init();
			}
		});
		...
		var szgIndex = -1;
		$("#txtPrice").blur(function() {
			if (this.value <= 40) setValue('txtMinMPrice', 1);
			if (this.value > 40 && this.value <= 80) setValue('txtMinMPrice', 1.5);
			if (this.value > 80 && this.value <= 120) setValue('txtMinMPrice', 2);
			if (this.value > 120 && this.value <= 200) setValue('txtMinMPrice', 3);
			if (this.value > 200 && this.value <= 500) setValue('txtMinMPrice', 4);
			if (this.value > 500 && this.value <= 1000) setValue('txtMinMPrice', 5);
			if (this.value > 1000 && this.value <= 1500) setValue('txtMinMPrice', 6);
			if (this.value > 1500) setValue('txtMinMPrice', 7);
		});
		...
		$("#btnCilentAdd").click(function() {
			...
			if (mtype == 2) {
				...
				if (searchDes == "") {
					...
					if (searchDes.length > 100) {
						artDialog({
							content: "对不起！" + destype + "搜索说明不能超过100个字符。",
							id: "alarm"
						}, function() {
							ToMao();
						});
			...
			if ($("#cbxIsMsg").is(':checked')) {
				if ($("#txtMessage").val().length > 200) {
					artDialog({
						content: "对不起！给接手人的规定好评内容请保持在200个字符以内。",
						id: "alarm"
					}, function() {}, null, function() {
						Init();
						ToMao2();
			...
			if ($("#cbxIsAddress").is(':checked')) {
				if ($("#cbxAddress").val().length > 100) {
					artDialog({
						content: "对不起！给接手人的规定收货地址请保持在100个字符以内。",
						id: "alarm"
					}, function() {}, null, function() {
						Init();
						ToMao2();
			...
			if ($("#cbxIsTip").is(':checked')) {
				...
				if (buycount != "") {
					...
					if (isNaN(buycount) || buycount < 1 || buycount > 999) {
						artDialog({
							content: "对不起！商品购买数量有效值为1到999之间的正整数。",
							id: "alarm"
						}, function() {
							$("#txtBuyCount").focus();
						});
			...
			if ($("#cbxIsTaoG").is(":checked")) {
				...
				if (isNaN(taoG) || taoG <= 0 || taoG > 300 || taoG % 10 != 0) {
					artDialog({
						content: "对不起，请输入正确的淘金币。淘金币为0到300之间的整数，且为10的倍数。",
						id: "alarm"
					}, function() {});
					return false;
				}
			...
			if (!validGoodsUrl || !ValidDomainF(validGoodsUrl)) {
				artDialog({
					content: "对不起，您还未输入商品URL地址或者地址有错误。（特别注意：此地址为您店铺需要买方拍的商品地址，而非淘宝店地址）",
					id: "alarm",
					yesText: "确定",
					noText: "查看URL地址帮助"
				}, function() {
					ToMao();
				} /*,function(){window.open("http://shua.114.acd8.com/");}*/ );
				$(".ui_close").hide();
				return false;
			} else {
				var dialog = artDialog({
					content: "正在验证您输入的商品url地址，请稍候...",
					id: "VGU",
					fixed: true,
					lock: true
				});
				$(".ui_close").hide();
				$.post("/ajax/checkshop.php", {
					"seller": szgIndex,
					"goodsUrl": goodsUrl
				}, function(result) {
					dialog.close();
					...
					if (result == 1) {
						...
						artDialog({
							content: "对不起！您输入的商品url地址对应的掌柜名与您在本平台选择的掌柜名不一致。请注意核对掌柜名是否正确（大小写以及繁体请注意区分），如有错误，请联系在线客服进行解决。",
							id: "alarm"
						}, function() {
							ToMao();
						});
						...
						artDialog({
							content: "地址为非淘宝正常商品地址  比如全球购地址，暂时无法发布任务。",
							id: "alarm"
						}, function() {
							ToMao();
						});
		...
		$("#cbxIsAddress").click(function() {
			if ($(this).attr("checked")) {
				if (getObj('isSign').checked || getObj('isawb').checked) {
					this.checked = false;
					artDialog({
						content: "对不起，真实签收任务、快递单号任务与规定收货地址不能同时选择",
						id: "alarm"
					}, function() {});
				}
				$("#cbxTip").show();
		...
		$("#cbxIsMsg").click(function() {
			if ($(this).attr("checked")) {
				if (getObj('isNoword').checked) {
					this.checked = false;
					artDialog({
						content: "对不起，规定好评内容与不带字好评任务不能同时选择",
						id: "alarm"
					}, function() {});
				}
			}
		...
		$("#cbxIsSetTime1,#cbxIsSetTime2").click(function() {
			if (this.checked) {
				...
				if ($("#cbxIsAudit").attr("checked")) {
					...
					artDialog({
						content: "设置任务延迟发布后，不论您是否在线您的任务都将在大厅显示，系统自动取消审核",
						id: "alarm"
					}, function() {});
				}
			}
...
function ValidPrice(price) {
	if (price > 4000) artDialog({
		content: "对不起！只能发布4000元以下的商品。",
		id: "alarm",
		yesText: "返回修改",
		noText: "取消"
	}, function() {
...
function ToMao() {
	$("html,body").animate({
		scrollTop: $("#mao1").offset().top
	}, 1000);
}

function ToMao2() {
	$("html,body").animate({
		scrollTop: $("#mao2").offset().top
	}, 1000);
}
...
function countPoint() {
	...
	if (getObj('cbxIsFMaxMCount').checked) {
		if (getObj('fmaxmc_2').checked) p += 0.2;
		else if (getObj('fmaxmc_3').checked) p += 1;
		else p += 0.5;
	}
...
function addconfirm() {
	...
	if (pointExt > 10 && price_count < 2000) {
		artDialog({
			content: "对不起！金额小于2000,追加悬赏麦点不能超过10个麦点。",
			id: "alarm",
			fixed: true,
			lock: true,
			noText: "返回修改"
		...
		artDialog({
			content: "对不起！金额大于2000,追加悬赏麦点不能超过20个麦点。",
			id: "alarm",
			fixed: true,
			lock: true,
			noText: "返回修改"
