<?php $_tplModify=filemtime('D:\xinyupingtai\cache\default\task\tao_taskOut.php');if(filemtime('D:\xinyupingtai\templates\default\headerBase.htm')>$_tplModify||filemtime('D:\xinyupingtai\templates\default\task\tao_info.htm')>$_tplModify||filemtime('D:\xinyupingtai\templates\default\quick.htm')>$_tplModify||filemtime('D:\xinyupingtai\templates\default\task\tab.htm')>$_tplModify||filemtime('D:\xinyupingtai\templates\default\footer.htm')>$_tplModify){include(template::load_base('D:\xinyupingtai\templates\default\task\tao_taskOut.htm','D:\xinyupingtai\cache\default\task\tao_taskOut.php',true));exit;}?><?php !defined("IN_JB")&&exit("error");$__tplUrl = '/templates/default/task/';echo '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<head>
...
<link href="/images/twitter.css" rel="stylesheet" type="text/css">
<style type="text/css">
...
.rwdt_xx .rwdt_ing_ {';echo 'position: absolute;top: 66px;left: 12px;';echo '}
.rwdt_xx .rwdt_ing_ a{';echo 'color:#3372A2';echo '}
.rwdt_xx .a_margin_left a{';echo 'margin-left:10px;';echo '}
.rwdt_xx a:hover{';echo 'color:rgb(255,85,0);text-decoration:underline;';echo '}
...
.yf_whexpress{';echo 'background: url(/images/tx_ico_2013530x.png) no-repeat;display: block;height: 30px;text-align: center;line-height: 30px;width: 158px;overflow: hidden;';echo '}
...
<style type="text/css">
.kd a:hover {';echo ' text-decoration:underline;';echo '}
.link_t {';echo ' display:block; width:52px; height:22px;line-height:22px;color:#a68d8b; background:url(/images/blue_2.gif) no-repeat; margin-bottom:2px;';echo '}
</style>
<style type="text/css">
.bq_menu{';echo '
	height:32px;
...
<div class="rwdt_bk">
		...
		<div class="rwdt_gg"><a href="';if(!$web_rewrite)echo $weburl2.'rewrite.php?rewrite=';echo '/member/userjob/" target="_blank"><img src="/images/renwujs.jpg"></a></div>
		<div class="rwdt_gg2"><a href="';if(!$web_rewrite)echo $weburl2.'rewrite.php?rewrite=';echo '/bbs/t1538/" target="_blank"><img src="/images/490x53-1.jpg"></a></div>
...
';echo '
		<div class="rwdt_xx">
		    ...
		    <p class="rwdt_marquee"><marquee onmouseover="this.stop()" onmouseout="this.start()" height="18" width="100%"><a href="';if(!$web_rewrite)echo $weburl2.'rewrite.php?rewrite=';echo '/bbs/t1247/" style="color:red;" target="_blank">被卡任务不拍？客服永远不会要求你操作任务，谨防上当</a>　　</marquee></p>
			<ul class="yfrw_sub">
				
				<li><a  href="javascript:;" onclick="window.location.href=\'';if(!$web_rewrite)echo $weburl2.'rewrite.php?rewrite=';echo '';echo $thisUrl;echo '&t=ing\'" ';if($t=='ing'){echo ' class="nov" ';}echo '>可处理(';echo $memberTask['outing1'];echo ')</a></li>
				
				<li>
				<a href="javascript:;" onclick="window.location.href=\'';if(!$web_rewrite)echo $weburl2.'rewrite.php?rewrite=';echo '';echo $thisUrl;echo '&t=waiting\'" ';if($t=='waiting'){echo ' class="nov" ';}echo '>等待接手付款(';echo $memberTask['outWaiting1'];echo ')</a></li>
				<li>
				<a href="javascript:;" onclick="window.location.href=\'';if(!$web_rewrite)echo $weburl2.'rewrite.php?rewrite=';echo '';echo $thisUrl;echo '&t=waiting2\'" ';if($t=='waiting2'){echo ' class="nov" ';}echo '>等待发货(';echo $memberTask['ining1'];echo ')</a></li>
				<li>
				<a href="javascript:;" onclick="window.location.href=\'';if(!$web_rewrite)echo $weburl2.'rewrite.php?rewrite=';echo '';echo $thisUrl;echo '&t=expire\'" ';if($t=='expire'){echo ' class="nov" ';}echo '>等待收货好评(';echo $memberTask['outExpire1'];echo ')</a></li>
				<li>
				<a href="javascript:;" onclick="window.location.href=\'';if(!$web_rewrite)echo $weburl2.'rewrite.php?rewrite=';echo '';echo $thisUrl;echo '&t=pause\'" ';if($t=='pause'){echo ' class="nov" ';}echo '>暂停中(';echo $memberTask['outPause1'];echo ')</a></li>
				<li>
				<a href="javascript:;" onclick="window.location.href=\'';if(!$web_rewrite)echo $weburl2.'rewrite.php?rewrite=';echo '';echo $thisUrl;echo '&t=complate\'" ';if($t=='complate'){echo ' class="nov" ';}echo '>已完成任务(';echo $memberTask['inComplate1'];echo ')</a></li>
				<li>
				<a href="javascript:;" onclick="window.location.href=\'';if(!$web_rewrite)echo $weburl2.'rewrite.php?rewrite=';echo '';echo $thisUrl;echo '&t=all\'" ';if($t=='all'){echo ' class="nov" ';}echo '>
				全部任务(';echo $memberTask['out1'];echo ')</a></li>
			...
			<div class="rwdt_ing_ a_margin_left"><ul><li><a href="javascript:getTaskSort(1,\'ing\');" onclick="">掌柜名排序</a><a href="javascript:getTaskSort(2,\'ing\');">发布时间排序</a></li></ul></div>
	
           <div class="task_search"><input name="task_keys" id="task_keys" onclick="if(this.value.indexOf(\'搜索任务ID\') >= 0)this.value=\'\';" value="搜索任务ID,掌柜,买号,QQ" type="text"><a href="javascript:;" class="search" onclick="task_search();">搜索</a></div>
		</div>
	<div class="cle"></div>
	<div class="reListTitle" id="taskio">			<!--开始-->
	...
	';if($tList){foreach($tList as $v){echo '
        <table class="yfrw_list" border="0" cellpadding="0" cellspacing="0" width="100%">
          ...
          <tr> 
			...
			<p style="height:30px; position:relative; top:5px;" id="';echo $v['id'];echo '">
				<a class="yf_addtime" href="';echo $thisUrl;echo '&addtime=';echo $v['id'];echo '">等待发布审核</a>
			</p>
			...
			<p style="height:60px; position:relative; top:5px;" id="';echo $v['id'];echo '">
			   <a href="javascript:;" onclick="dialog(600,500,\'大麦户平台任务满意度评分\',\'/dialog/grade/?sid=';echo $v['id'];echo '\');return false;" style="margin-left:5px;" class="yf_pinjia float_r" title="给接手方打分" id="grade_';echo $v['id'];echo '">评价</a>
		  ...
		  <tr>
            ...
            <td colspan="9" class="yfrw_listxia" align="right" valign="middle">
              ...
              <tbody>
			  <tr>
			    <td style="padding-left:20px; color:#3372A2;" align="left" width="40%">客服不会主动联系操作任务、平台没有充值活动谨防假冒</td>
...
var webnoticeurl = "";
var webnoticetit = "";
var thisUrl=\'/task/tao/?m=taskOut\';
</script>
<script type="text/javascript" src="/images/service.js"></script>
<script language="javascript">
var time_ = {';echo 'cache:2,start:0';echo '};
function ly_check()
{';echo '
	if($(this).attr(\'checked\') == \'checked\'){';echo '
		$(this).attr(\'checked\',\'\');$("#taskio input[name=\'ID_\']").attr("checked", "");
	';echo '}else{';echo '
		$(this).attr(\'checked\',\'checked\');$("#taskio input[name=\'ID_\']").attr("checked", "checked"); 
	';echo '}
';echo '}

function oneConfirm(){';echo '
	var url =\'?m=taskOut&confirm=\';
	allPut(url);
';echo '}

function oneSend(){';echo '
	var url =\'?m=taskOut&send=\';
	allPut(url);
';echo '}
...
{';echo '	 
	var id=[],i=0;
	artDialog({';echo 'title:\'提示\',content:\'正在提交数据，请稍后...\'';echo '});
	$("[name=\'ID_\']:checked").each(function(){';echo '
	    id[i]=this.value;
		i++;
	';echo '});
	id = encodeURIComponent(id.join(\',\'));
	$.ajax({';echo '
			async:false,
			url:u+id,
			success:function(request){';echo '
...
function task_search(){';echo '
	  ...
	  ';echo '}
';echo '}
var _sortStatusz=\'asc\';
var _sortStatusf=\'desc\';
function getTaskSort(type,t){';echo '
	var str=\'\';
	if (type==1){';echo '
		str +="nickname="+_sortStatusz;
		_sortStatusz = (_sortStatusz==\'asc\')?\'desc\':\'asc\'
	';echo '}else{';echo '
		str +="stimestamp="+_sortStatusf;
		_sortStatusf = (_sortStatusf==\'asc\')?\'desc\':\'asc\'
...
function goPage(n) {';echo '
	...
	';echo '}
    getObj(\'taskio\').innerHTML = \'<div class=\\\'submiting\\\'>任务加载中.....</div>\';
    getTask(pageSearch);
';echo '}
function ShowRemark(id,ramark){';echo '
    remarkHtm = "<textarea id=\\"txtremark\\" cols=\\"50\\" rows=\\"10\\" style=\\"width:200px;height:100px;\\">"+ ramark +"</textarea>";
    artDialog({';echo 'content:remarkHtm,id:"alarm",fixed:true,yesText:"保存并关闭",noText:"关闭"';echo '},
        function(){';echo '
            ...
            if(ramark.length<=100){';echo '
                $.post("/ajax/updateremark.php",{';echo '"taskid":id,"ramark":ramark';echo '}, function(result){';echo '
                    if(result>0){';echo '
                        goPage();
                    ';echo '}else{';echo '
					alert(result);
...
<script type="text/javascript">
function IsConfirmTask(plat) {';echo '
	var aa = $.dialog({';echo '
		title: false,
		content: "<img width=520 height=211 src=\'/images/1232.gif\'>",
		fixed: true,
		lock: true,
		okValue: \'确定\',
		cancelValue: \'取消\',
		ok: function () {';echo '
			window.location.href=plat;return true;
		';echo '},
		cancel: function () {';echo ' return true; ';echo '}
	';echo '});
	$(\'.d-content\').css("padding","0");
	$(\'.d-titleBar\').empty();
	return false;
';echo '}
function getTask(qs) {';echo '
    //alert(qs);
    var url = \'/ajax/getTaskinOut.php?type=1&au=8&m=taskOut&\' + qs+"&t=';echo $t;echo '";
    $(\'#taskio\').html(\'<div class=\\\'submiting clear\\\'>加载中.....</div>\');
    $.ajax({';echo '
        type : \'GET\',
        url  : url,
        ...
        ';echo '},
        error:function(){';echo '
            $(\'#taskio\').html(\'<p>&nbsp;</p><p>尊敬的会员，您遇到了一个未知的错误，请稍后重新点击。　　<a href="\'+weburl2+\'">返回主页</a></p>\');
        ';echo '}
    ';echo '});
';echo '}
</script>
