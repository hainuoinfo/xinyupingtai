$(document).ready(function(){
    ...
    if( curG < 1 ){
        ...
        $("#btnCilentAdd").click(function(){
            ...
            if(szgIndex==-1){
                artDialog({content:"对不起！请选择一个拍拍掌柜名。",id:"alarm"},function(){},null,function(){Init();ToMao();});
		        return false;
            }
	        ...
	        if(!IsURL(validGoodsUrl)|| !ValidDomainF(validGoodsUrl) || validGoodsUrl.indexOf("combo")>0){
	            ...
	            $.post("/ajax/checkshop2.php",{"seller":szgIndex,"goodsUrl":goodsUrl}, function(result){
	                dialog.close();
	                //$(".ui_close").show(); 
	                if(result=='0'){
                           ...
                           artDialog({content:"对不起！您提交的参数错误，请重选选择后提交。",id:"alarm"},function(){ToMao();});
                            ...
                            artDialog({content:"对不起！您选择的拍拍掌柜名有误。",id:"alarm"},function(){ToMao();});
					} else if (result=='11'){
                            artDialog({content:"对不起！您输入的商品url地址对应的掌柜名与您在本平台选择的掌柜名不一致。",id:"alarm"},function(){ToMao();});
					} else if (result=='12'){
       ...
       /* $("#cbxIsSetTime1").click(function(){
        ...
        $("#cbxIsAddress").click(function(){
                if($(this).attr("checked")){
					...
					this.checked=false;
					artDialog({content:"对不起，真实签收任务与规定好评内容不能同时选择",id:"alarm"},function(){});
					}
					$("#cbxTip").show();
				}else{
					$("#cbxTip").hide();
...
function ValidPrice(price){
    if( price >= 3000 )
            artDialog({content:"对不起！只能发布3000元以下的商品。",id:"alarm",yesText:"返回修改",noText:"取消"},
            function(){Init();ToMao();},
...
function countPoint() {
   ...
   if(getObj('cbxIsSetTime1').checked) p += 0.1;
   if(getObj('cbxIsSetTime2').checked) p += 0.1;
   p=p.toFixed(2);
   return p;
}
