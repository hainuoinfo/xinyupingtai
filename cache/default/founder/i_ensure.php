<?php $_tplModify=filemtime('D:\damaihu\xinyupingtai7\cache\default\founder\i_ensure.php');if(filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\h.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\header.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\f.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\footer.htm')>$_tplModify){include(template::load_base('D:\damaihu\xinyupingtai7\templates\default\founder\i_ensure.htm','D:\damaihu\xinyupingtai7\cache\default\founder\i_ensure.php',true));exit;}?><?php !defined("IN_JB")&&exit("error");$__tplUrl = '/templates/default/founder/';echo '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
';echo $sys_hash_code;echo '
	<table class="tab_view" style="width:800px">
		...
		<tr>
			<th>任务ID</th>
			<td>';echo $ensure['tid'];echo '</td>
		</tr>
		<tr>
			<th>所属区域</th>
			<td>';echo language::get('qu'.$ensure['type']);echo '</td>
		</tr>
		<tr>
			<th>维权人</th>
			<td>';echo $ensure['fusername'];echo '</td>
		</tr>
		<tr>
			<th>被维权人</th>
			<td>';echo $ensure['tusername'];echo '</td>
		</tr>
		<tr>
			<th>维权标题</th>
			<td>';echo $ensure['title'];echo '</td>
		</tr>
		<tr>
			<th>维权内容</th>
			<td>
				';if($ensureMessage['0']){foreach($ensureMessage['0'] as $v){echo '
				[';echo date('Y-m-d H:i:s',$v['timestamp']);echo ']<br />';echo $v['message'];echo '<br /><br />
				';}}echo '
			</td>
		</tr>
		';if($ensureMessage[1]){echo '
		<tr>
			<th>辩解内容</th>
			<td>
				';if($ensureMessage[1]){foreach($ensureMessage[1] as $v){echo '
				[';echo date('Y-m-d H:i:s',$v['timestamp']);echo ']<br />';echo $v['message'];echo '<br /><br />
				';}}echo '
			</td>
		</tr>
		...
		<tr>
			<th>维权时间</th>
			<td>';echo date('Y-m-d H:i:s',$ensure['timestamp1']);echo '</td>
		</tr>
		<tr>
			<th>状态</th>
			<td>';echo $cStatus[$ensure['status']];echo '</td>
		</tr>
		<tr>
			<th>买家担保金额</th>
			<td>';echo $buyerInfo['attach']['ensureMoney'];echo '</td>
		</tr>
		';if(!in_array($ensure['status'],array(1,2,3))){echo '
		<tr>
			<th>扣除担保金给卖家</th>
		...
		<!--<tr>
			<th>退回金额</th>
			<td>￥';echo $ensure['price'];echo '</td>
		</tr>
		<tr>
			<th>退回发布点</th>
			<td>';echo $ensure['point'];echo '</td>
		</tr>
		<tr>
			<th>给发布方增加金钱</th>
			<td>￥';echo $ensure['sprice'];echo '</td>
		</tr>
		<tr>
			<th>给发布方增加发布点</th>
			<td>￥';echo $ensure['spoint'];echo '</td>
		</tr>
		<tr>
			<th>给接手方增加金钱</th>
			<td>￥';echo $ensure['bprice'];echo '</td>
		</tr>
		<tr>
			<th>给接手方增加发布点</th>
			<td>￥';echo $ensure['bpoint'];echo '</td>
		</tr>-->
		';}echo '
		<tr>
			<th>操作</th>
			<td>';if(!in_array($ensure['status'],array(1,2,3))){echo '
		...
		<tr>
			<th>任务详情</th>
			<td><a href="';echo $admin_url;echo '?action=i&operation=task&method=view&sid=';echo $ensure['tid'];echo '" target="_blank">点击查看</a></td>
		</tr>
		<tr>
			<th></th>
