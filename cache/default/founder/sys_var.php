<?php $_tplModify=filemtime('D:\damaihu\xinyupingtai7\cache\default\founder\sys_var.php');if(filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\h.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\header.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\f.htm')>$_tplModify||filemtime('D:\damaihu\xinyupingtai7\templates\default\founder\footer.htm')>$_tplModify){include(template::load_base('D:\damaihu\xinyupingtai7\templates\default\founder\sys_var.htm','D:\damaihu\xinyupingtai7\cache\default\founder\sys_var.php',true));exit;}?><?php !defined("IN_JB")&&exit("error");$__tplUrl = '/templates/default/founder/';echo '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
';echo '<form method="post" enctype="application/x-www-form-urlencoded" onsubmit="return confirm(\'确定提交吗？\')">';echo $sys_hash_code;echo '
	<table class="tb tb2 fixpadding"><tr class="header"><th>删？</th><th>变量名</th><th>变量值</th><th>备注</th><th></th></tr>';if($list){foreach($list as $k=>$v){echo '<tr class="hover"><td><input type="checkbox" name="del[]" value="';echo $v['id'];echo '" class="checkbox" /></td><td>';echo $v['key'];echo '</td><td>';echo $v['val'];echo '</td><td>';echo $v['remark'];echo '</td><td>[<a href="';echo $base_url;echo '&edit=';echo $v['id'];echo '">编辑</a>]</td></tr>';}}echo '<tr><td colspan="5"><input type="checkbox" id="checkDel" onclick="check_all(this,\'del[]\')" class="checkbox" /><label for="checkDel">全选</label></td></tr><tr><td colspan="5">';echo $multipage;echo '</td></tr><tr><td colspan="5"><input type="submit" value="提交" class="btn" /></td></tr></table></form>';echo '
...
';echo '<form method="post" enctype="application/x-www-form-urlencoded">';echo $sys_hash_code;echo '';if($update){echo '<input type="hidden" name="isEdit" value="yes" />';}echo '
	<table class="tb tb2 fixpadding"><tr class="hover"><td width="48" align="right" valign="top">变量名</td><td width="450"><input type="text" name="f_key" id="f_key" value="';echo $f_key;echo '" class="txt" style="width:128px" maxlength="32" preg="null=请输入变量名" /></td><td id="f_key_tip"></td></tr><tr class="hover"><td width="48" align="right" valign="top">变量值</td><td width="450"><textarea name="f_val" id="f_val" class="tarea" preg="null=请输入变量值">';echo htmlspecialchars($f_val);echo '</textarea></td><td id="f_val_tip"></td></tr><tr class="hover"><td width="48" align="right" valign="top">备注</td><td width="450"><input type="text" name="f_remark" id="f_remark" value="';echo $f_remark;echo '" class="txt" style="width:128px" maxlength="32" preg="null=请输入变量说明" /></td><td id="f_remark_tip"></td></tr><tr><td></td><td><input type="submit" value="';if($update){echo '编辑';}else{echo '提交';}echo '" class="btn" /></td></tr></table></form>';echo '
';}echo '
';echo '</div>
';echo '</body>
</html>';?>